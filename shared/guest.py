"""Guest(dom) — THE host-side I/O core for one libvirt guest.

One class owns every virsh / qemu-guest-agent round-trip: exec (PowerShell or
any binary), guest-file read/write, screenshot + pixel crop, input injection
(keys / clicks / AHK), VM lifecycle. Grown in forge/guest.py ("the healer's
host-helpers generalized"), promoted to shared/ so the healer's host_agent,
forge, harvest, and the RE tools all drive guests through the SAME plumbing
instead of five hand-rolled copies (REFACTOR P0.1). `forge.guest` remains as a
re-export shim for one release.

All methods are SYNCHRONOUS subprocess calls — run them via
`loop.run_in_executor` from async code (as the callers do) so the event loop
...
class Guest:
    def __init__(self, dom: str, width: int = 1920, height: int = 1080, *,
                 virsh_timeout: float = 8.0, agent_retries: int = 4,
                 exec_poll_s: float = 0.2) -> None:
        self.dom = dom
        self.width = width
        self.height = height
        # retry/poll knobs so latency-sensitive callers (host_agent) can tighten them
        self.virsh_timeout = virsh_timeout
        self.agent_retries = agent_retries
        self.exec_poll_s = exec_poll_s
        # per-domain scratch file so two guests don't clobber each other's frame
        # (name kept from the forge era; downstream tools reference it)
        self.ppm = f"/tmp/ib_forge_{dom}.ppm"

    # -- low level ---------------------------------------------------------
    def _virsh(self, *args, timeout: float | None = None) -> subprocess.CompletedProcess:
        return subprocess.run(VIRSH + list(args), capture_output=True, text=True,
                              timeout=timeout or self.virsh_timeout)
    ...
    def agent_cmd(self, cmd: dict, timeout: float = 6.0) -> dict:
        """qemu-agent-command with retries; returns the parsed ["return"] dict.
        The guest-agent channel hiccups (empty/garbled reply) under concurrent
        load; a single miss would otherwise fail a whole inject — retry a few
        times before raising (host_agent's proven pattern)."""
        last = ""
        for _ in range(self.agent_retries):
            r = self._virsh("qemu-agent-command", self.dom, json.dumps(cmd),
                            timeout=timeout)
    ...
    def run_ahk(self, script: str) -> bool:
        ...
        b = base64.b64encode(script.encode("utf-8")).decode("ascii")
    ...
    def file_write(self, path: str, data: bytes) -> None:
        """Write bytes to a guest file via the guest-agent FILE API (open/write/
        close). No process spawn — this is the fast path the healer's key inject
        uses (keycmd.txt / keys.txt). Raises on failure so callers can fall back."""
        h = self.agent_cmd({"execute": "guest-file-open",
                            "arguments": {"path": path, "mode": "w"}})
        self.agent_cmd({"execute": "guest-file-write", "arguments": {
            "handle": h, "buf-b64": base64.b64encode(data).decode()}})
        self.agent_cmd({"execute": "guest-file-close", "arguments": {"handle": h}})

    def push_bytes(self, data: bytes, guest_path: str) -> bool:
        """Copy bytes into a guest file via chunked PowerShell (no shared folder).
        Slower than file_write but proven for LARGE payloads: one guest-exec command
        has a hard length limit (~a few KB), so the base64 is appended to a staging
        .b64 in pieces then decoded — inlining it all in a single command silently
        fails (which stopped sync_reflex from ever landing). True on confirmed write."""
        b = base64.b64encode(data).decode("ascii")   # b64 alphabet has no quotes -> safe inline
    ...
    def push_file(self, local_path, guest_path: str) -> bool:
        """Copy a host-side file into the guest (push_bytes of its contents). Used to
        keep in-guest agent code in lockstep with the repo so guests never run stale
        logic. Returns True on a confirmed write."""
        try:
            data = Path(local_path).read_bytes()
        except Exception:
            return False
        return self.push_bytes(data, guest_path)

    def push_text(self, guest_path: str, text: str) -> bool:
        """Write a UTF-8 text file in the guest (handles quotes/JSON safely)."""
        return self.push_bytes(text.encode("utf-8"), guest_path)
    ...
    def sync_reflex(self) -> bool:
        ...
        copy lives in the repo's guest_agent/ package."""
        src = Path(__file__).resolve().parent.parent / "guest_agent" / "craft_reflex.py"
    ...
    # -- guest exec (guest-exec; gexec.py pattern) --------------------------
    def exec_ps(self, ps: str, wait: bool = True, poll: int = 60,
                poll_s: float | None = None) -> str | None:
        ...
        for _ in range(poll):
            ...
            time.sleep(poll_s if poll_s is not None else self.exec_poll_s)
        return None

    def read_ps(self, ps: str) -> str | None:
        """exec_ps tuned for the sense/inject hot paths (host_agent's _guest_read):
        tighter poll, shorter overall deadline (~4.5s vs 12s)."""
        return self.exec_ps(ps, poll=30, poll_s=0.15)

    def exec_start(self, path: str, args: list[str]) -> None:
        """Fire-and-forget guest-exec of any binary (no output capture)."""
        self._virsh("qemu-agent-command", self.dom, json.dumps({
            "execute": "guest-exec",
            "arguments": {"path": path, "arg": args}}))

    def exec_out(self, path: str, args: list[str], wait: float = 8.0,
                 poll_s: float = 0.3) -> str:
        """Run any guest binary with output capture, poll to completion, return
        stdout+stderr ("" on failure/timeout). The harvest dashboard's _gx pattern
        (in-guest python readers etc.)."""
        try:
            ret = self.agent_cmd({"execute": "guest-exec", "arguments": {
        ...
        while time.time() - t0 < wait:
            time.sleep(poll_s)
            try:
                st = self.agent_cmd({"execute": "guest-exec-status",
                                     "arguments": {"pid": pid}})
            except Exception:
                continue
            if st.get("exited"):
                for k in ("out-data", "err-data"):
                    if st.get(k):
                        out += base64.b64decode(st[k]).decode("utf-8", "replace")
                break
        return out
